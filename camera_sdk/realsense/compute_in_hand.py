...
def func(data_dir=None):
    """
    :param data_dir: 本次标定数据目录（相对项目根或绝对路径），如 eye_hand_data/data20260512。
                     为 None 时使用 eye_hand_data 下日期最新的文件夹。
    """
    path = os.path.dirname(os.path.abspath(__file__))
    if data_dir is None:
        latest = find_latest_data_folder(current_path)
        if latest is None:
            raise RuntimeError("eye_hand_data 下未找到 dataYYYYMMDD 形式的文件夹。")
        img_dir = os.path.join(path, "eye_hand_data", latest)
    else:
        img_dir = os.path.normpath(data_dir if os.path.isabs(data_dir) else os.path.join(path, data_dir))

    file_path = os.path.join(img_dir, "poses.txt")
    ...
    images_num = [f for f in os.listdir(img_dir) if f.endswith('.jpg')]

    for i in range(1, len(images_num) + 1):   #标定好的图片在images_path路径下，从0.jpg到x.jpg

        image_file = os.path.join(img_dir, f"{i}.jpg")
    ...
    if N == 0:
        raise RuntimeError(
            "未在任意一张图中检测到棋盘角点，无法标定。请检查 config.yaml 的 XX、YY 与图像质量。"
        )
    if N < 3:
        raise RuntimeError(f"仅 {N} 张图检测到角点，至少需要 3 张，建议 10 张以上。")

    # 标定,得到图案在相机坐标系下的位姿
    ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(obj_points, img_points, size, None, None)
    logger_.info(f"相机标定平均重投影误差 ret={ret:.4f} 像素")
