...
Keyboard:
    ...
    [s]     Save PNG (./out.png)
    [t]     Save debug bundle to test_data/pyglet (depth + point cloud assets)
...
DEBUG_SAVE_ROOT = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "test_data", "pyglet"
)


def save_pyglet_debug_bundle(
    depth_frame,
    color_image_rgb,
    colorized_depth_bgr,
    rs_points,
    ...
    """保存深度 / 彩色 / RealSense PLY / 顶点网格，便于与 hzy_test 等对比。"""
    ...
    # 配置里彩色流为 rgb8，存盘转成 BGR 便于 Windows 看图软件颜色正常
    ...
    meta = (
        f"pipeline: pyglet viewer (no rs.align; optional decimate + post filters)\n"
    ...
    print(f"[pyglet debug] 已写入 {base}_* → {DEBUG_SAVE_ROOT}")
...
# 检查数据流是否正常
pipeline_wrapper = rs.pipeline_wrapper(pipeline)        # 创建管道包装器
pipeline_profile = config.resolve(pipeline_wrapper)     # 对目标设备进行预解析，并返回设备配置对象
device = pipeline_profile.get_device()                  # 从解析结果中获取设备对象
...
profile = pipeline.get_active_profile()     # 返回当前实际选中的设备数据

depth_profile = rs.video_stream_profile(profile.get_stream(rs.stream.depth))    # 从当前配置中取出深度类指针（深度配置描述）
depth_intrinsics = depth_profile.get_intrinsics()       # 返回深度相机内参（fx,fy,px,py,width,height）
w, h = depth_intrinsics.width, depth_intrinsics.height    # 深度图的宽度和高度
